 ...
 * 支持说明：
 * <li>只支持POST和GET方法</li>
 * <li>参数只支持从HEADER、FORM表单、PATH中和JSON数据中取</li>
 * <p>
 * 用法说明：<br/>
 * ResourceFactory resourceFactory = ResourceFactory.build("http://127.0.0.1:8080/ws/");<br/>
 * 通过以下方法得到资源类的实例后就可以与调用普通java方法一样调用资源了（PS：将Resource替换为实际的资源类即可）<br/>
 * Resource resource = resourceFactory.build(Resource.class);<br/>
...
public class ResourceFactory {
    ...
    private static class CglibHTTPProxy implements MethodInterceptor {
         ...
         * @return 代理
         */
        public static CglibHTTPProxy build(String baseUrl) {
        ...
        private IHttpRequestBase build(Object o, Object[] objects, Method method) {
            ...
            for (int i = 0; i < parameters.length; i++) {
                ...
                if (empty) {
                    //如果遍历后是空并且参数只有一个，说明是要接收一个json类型的参数，否则说明异常了
                    if (parameters.length == 1) {
                        logger.debug("参数是json类型的参数");
                        data = parser.toJson(value);
                    } else {
                        //一般不会抛出，除非遇到BeanParam等
                        throw new RuntimeException("参数的注解为：" + parser.toJson(annotations) + "；没有合适的注解或者暂时不支持");
                    }
                }
            }
