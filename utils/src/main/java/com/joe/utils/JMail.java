...
public class JMail {
	 ...
	 */
	public static boolean sendEmail(String body, String to, String title) {
	 ...
	 * @return 如果发送成功则返回<code>true</code>
	 */
	public static boolean sendEmail(String body, String to, String title, FilePart[] fileParts) {
		if (StringUtils.isEmpty(to) || (StringUtils.isEmpty(body) && fileParts == null)) {
		...
		try {
			...
			if (fileParts != null && fileParts.length != 0) {
				for (FilePart filePart : fileParts) {
					MimeBodyPart attach = new MimeBodyPart(); // 表示附件1
